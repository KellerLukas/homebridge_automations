    ...
    def restore(self, transition_time_deciseconds: Optional[int] = None):
        for id in self.light_ids:
            self.controller.set_state(
                ...
                transition_time_deciseconds=transition_time_deciseconds,
            )
        self.valid_until = (
            time.time() + transition_time_deciseconds / 10
            if transition_time_deciseconds
            else time.time()
        )
    ...
    def set_states(self, states: {}, transition_time_decisenconds: Optional[int]):
        for id in self.light_ids:
            self.controller.set_state(
                light_id=id,
                state=states[id],
                transition_time_deciseconds=transition_time_decisenconds,
            )
